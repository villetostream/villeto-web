    ...
    <section id="trust" className="bg-[#edf2f0] py-20 text-[#111714] sm:py-28">
      <Container>
        <Reveal className="grid gap-5 md:grid-cols-[0.72fr_1.28fr] md:items-end">
          <p className="type-eyebrow font-semibold uppercase text-[#087f70]">{trustSection.eyebrow}</p>
          <div>
            <h2 className="max-w-[760px] text-[length:var(--fs-h2)] font-semibold leading-[1.08]">{trustSection.heading}</h2>
            <p className="type-section-copy mt-4 max-w-[620px] text-[#53605a]">{trustSection.subhead}</p>
          </div>
        </Reveal>
        ...
        <Reveal className="mt-12 overflow-hidden rounded-[8px] border border-black/[0.1] bg-white shadow-[0_28px_75px_-58px_rgba(5,20,15,0.5)]">
          <div className="flex flex-col gap-3 border-b border-black/[0.08] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
            <div className="flex items-center gap-3">
              ...
              <div><p className="type-ui font-semibold">Assurance matrix</p><p className="type-meta mt-0.5 text-[#6b7671]">Controls remain attached to the workflow.</p></div>
            </div>
            <span className="type-meta flex items-center gap-2 font-semibold text-[#087f70]"><span className="size-1.5 rounded-full bg-[#0ea894]" />{trustSection.status}</span>
          </div>
            ...
            {trustItems.map((item, index) => {
              ...
              return (
                <div key={item.id} className="grid grid-cols-[36px_minmax(0,1fr)_auto] gap-x-3 gap-y-2 px-5 py-5 sm:grid-cols-[42px_0.7fr_1.3fr_auto] sm:items-center sm:gap-4 sm:px-7">
                  ...
                  <div><span className="type-meta font-semibold text-[#087f70]">0{index + 1}</span><h3 className="type-card-title mt-1 font-semibold">{item.title}</h3></div>
                  <p className="type-card-copy col-span-2 col-start-2 text-[#5f6a65] sm:col-span-1 sm:col-start-auto">{item.body}</p>
                  <span className="type-meta col-start-3 row-start-1 w-fit rounded-[6px] bg-[#e6f5f2] px-2.5 py-1.5 font-semibold text-[#087f70] sm:col-start-auto sm:row-start-auto">{item.status}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="grid border-t border-black/[0.08] bg-[#f7f9f8] grid-cols-2 sm:grid-cols-4">
            {trustSignals.map((signal) => (
              <div key={signal} className="type-ui flex min-h-[58px] items-center gap-2 border-b border-r border-black/[0.06] px-4 font-medium text-[#53605a] last:border-r-0 sm:border-b-0">
                <CheckMark />{signal}
              </div>
            ))}
          </div>
