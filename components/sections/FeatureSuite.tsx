  ...
  return (
    <div className="overflow-hidden rounded-[8px] border border-black/[0.08] bg-white text-[#111714] shadow-[0_28px_65px_-44px_rgba(5,20,15,0.6)]">
      <div className="flex items-center justify-between border-b border-black/[0.06] px-5 py-4">
        <div className="flex items-center gap-2"><ClipboardList className="size-4 text-[#087f70]" /><span className="type-ui font-semibold">Purchase request</span></div>
        <span className="type-meta rounded-[6px] bg-[#e7f6f2] px-2 py-1 font-semibold text-[#087f70]">Complete</span>
      </div>
      <div className="grid gap-4 p-5 sm:grid-cols-[1.15fr_0.85fr] sm:p-6">
        <div className="flex min-h-[190px] flex-col justify-between rounded-[8px] bg-[#f4f6f5] p-5">
          <div className="flex items-start gap-3">
            ...
            <div><p className="type-card-copy font-semibold">Aster Cloud</p><p className="type-meta mt-1 text-[#77807c]">Analytics workspace · Annual</p></div>
          </div>
          <div className="flex items-end justify-between border-t border-black/[0.06] pt-4"><span className="type-meta text-[#77807c]">Operations team</span><span className="text-[17px] font-semibold">$6,800</span></div>
        </div>
          ...
          {[
            ...
            return (
              <motion.div key={label as string} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.08 }} className="flex items-center justify-between rounded-[8px] border border-black/[0.06] px-3 py-3">
                <span className="type-meta flex items-center gap-2 font-medium text-[#53605a]"><RowIcon className="size-3.5 text-[#087f70]" />{label as string}</span>
...
function VendorVisual() {
  ...
  return (
    <div className="overflow-hidden rounded-[8px] border border-black/[0.08] bg-white text-[#111714] shadow-[0_28px_65px_-44px_rgba(5,20,15,0.6)]">
      <div className="flex items-center justify-between border-b border-black/[0.06] px-5 py-4">
        <span className="type-ui font-semibold">Vendor profile</span>
        <span className="type-meta flex items-center gap-1.5 rounded-[6px] bg-[#e7f6f2] px-2 py-1 font-semibold text-[#087f70]"><CheckCircle2 className="size-3" />Verified</span>
      </div>
      <div className="grid gap-5 p-5 sm:grid-cols-[0.8fr_1.2fr] sm:p-6">
        <div className="rounded-[8px] bg-[#111714] p-5 text-white">
          ...
          <p className="type-card-copy mt-5 font-semibold">Aster Cloud Systems Ltd.</p>
          <p className="type-meta mt-1 text-white/50">Software · United States</p>
          <div className="mt-6 border-t border-white/10 pt-4"><p className="type-meta uppercase text-white/40">Business owner</p><p className="type-ui mt-1 font-medium">Amara · Operations</p></div>
        </div>
        <div className="divide-y divide-black/[0.06]">
          {checks.map((item, index) => (
            <motion.div key={item.label} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }} className="flex items-center justify-between py-4 first:pt-1 last:pb-1">
              <div><p className="type-ui font-semibold">{item.label}</p><p className="type-meta mt-1 text-[#77807c]">{item.detail}</p></div>
  ...
  return (
    <div className="grid overflow-hidden rounded-[8px] border border-white/10 bg-[#0b100e] text-white shadow-[0_28px_65px_-44px_rgba(0,0,0,0.9)] sm:grid-cols-[0.72fr_1.28fr]">
      <div className="flex flex-col items-center justify-center border-b border-white/10 p-6 sm:border-b-0 sm:border-r">
        ...
        <p className="type-ui font-semibold">Policy engine</p>
        <p className="type-meta mt-1 text-white/40">Evaluating before spend</p>
      </div>
      <div className="p-5 sm:p-6">
        <div className="flex items-center justify-between border-b border-white/10 pb-4"><span className="type-ui font-semibold">Aster Cloud · $6,800</span><span className="type-meta text-[#6edbca]">3 controls</span></div>
        <div className="divide-y divide-white/[0.07]">
          {checks.map((item, index) => <motion.div key={item} initial={reduceMotion ? false : { opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="flex items-center justify-between py-3.5"><span className="type-meta text-white/65">{item}</span><span className="flex size-5 items-center justify-center rounded-full bg-[#15352f] text-[#6edbca]"><Check className="size-3" /></span></motion.div>)}
        </div>
        <div className="mt-2 flex items-center justify-between rounded-[8px] bg-[#15352f] px-3 py-2.5 text-[#8ce5d7]"><span className="type-meta font-semibold">Cleared automatically</span><CheckCircle2 className="size-3.5" /></div>
      </div>
    </div>
  );
}
...
function RecordVisual() {
  ...
  return (
    <div className="overflow-hidden rounded-[8px] border border-black/[0.08] bg-white text-[#111714] shadow-[0_28px_65px_-44px_rgba(5,20,15,0.6)]">
      <div className="flex items-center justify-between border-b border-black/[0.06] px-5 py-4"><span className="type-ui font-semibold">Financial record</span><span className="type-meta font-medium text-[#77807c]">Complete</span></div>
      <div className="p-5 sm:p-6">
        {events.map((event, index) => {
          ...
          return (
            <motion.div key={event.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="relative flex gap-3 pb-5 last:pb-0">
              ...
              <div className="min-w-0 flex-1 border-b border-black/[0.05] pb-4 last:border-0"><div className="flex items-center justify-between"><p className="type-ui font-semibold">{event.label}</p><CheckCircle2 className="size-3.5 text-[#0ea894]" /></div><p className="type-meta mt-1 text-[#77807c]">{event.detail}</p></div>
            </motion.div>
          );
        })}
      </div>
    ...
    <section id="products" className="bg-[var(--bg-canvas)] py-20 sm:py-28">
      <Container>
        <Reveal className="grid gap-5 md:grid-cols-[0.72fr_1.28fr] md:items-end">
          <p className="type-eyebrow font-semibold uppercase text-[var(--accent-text)]">The spend lifecycle</p>
          <div>
            <h2 className="max-w-[720px] text-[length:var(--fs-h2)] font-semibold text-[var(--text-primary)]">Four moments. One connected record.</h2>
            <p className="type-section-copy mt-4 max-w-[650px] text-[var(--text-secondary)]">Move from request to vendor review, control, and reconciliation without rebuilding context at every step.</p>
          </div>
        </Reveal>
        ...
        <div className="mt-12 grid overflow-hidden rounded-[8px] border border-[var(--border-hairline)] bg-[var(--bg-surface)] md:grid-cols-[0.72fr_1.28fr]">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="type-meta font-semibold text-[var(--accent-text)]">{item.number}</span>
                    <span className={`type-ui font-semibold uppercase ${selected ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}>{item.label}</span>
                  </div>
                  <AnimatePresence initial={false}>
                    {selected && (
                      <motion.div initial={reduceMotion ? false : { opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={reduceMotion ? undefined : { opacity: 0, height: 0 }} transition={{ duration: 0.22 }} className="overflow-hidden pl-11">
                        <h3 className="mt-4 text-[length:var(--fs-h3)] font-semibold text-[var(--text-primary)]">{item.title}</h3>
                        <p className="type-card-copy mt-2 max-w-[340px] text-[var(--text-secondary)]">{item.summary}</p>
                        <div className="mt-4 flex flex-wrap gap-x-3 gap-y-2">
                          {item.signals.map((signal) => <span key={signal} className="type-meta flex items-center gap-1.5 font-medium text-[var(--text-primary)]"><Check className="size-3 text-[var(--accent)]" />{signal}</span>)}
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
          ...
          <div className="flex min-h-[410px] flex-col justify-between p-5 sm:p-7 md:min-h-[560px] md:p-9">
            <div className="flex items-center justify-between">
              <span className="type-meta font-semibold uppercase text-[var(--text-secondary)]">Stage {active + 1} of {stages.length}</span>
              <span className="type-meta flex items-center gap-2 font-medium text-[var(--accent-text)]"><span className="size-1.5 rounded-full bg-[var(--accent)]" />Context carried forward</span>
            </div>
            <div className="my-7">
            ...
            <div className="flex items-center justify-between border-t border-[var(--border-hairline)] pt-4">
              <span className="type-ui text-[var(--text-secondary)]">{stage.number} · {stage.label}</span>
              <button type="button" onClick={() => setActive((active + 1) % stages.length)} className="type-ui inline-flex items-center gap-2 font-semibold text-[var(--accent-text)]">
        ...
        <div className="flex justify-end pt-6"><a href="#workflow" className="type-ui inline-flex items-center gap-2 font-semibold text-[var(--accent-text)]">Explore the product workflow <ArrowRight className="size-4" /></a></div>
      </Container>
    </section>
  );
}
