  ...
  return (
    <div className="overflow-hidden rounded-[8px] border border-white/10 bg-[#111714] shadow-[0_38px_90px_-48px_rgba(0,0,0,0.9)]">
      <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
        <div className="flex items-center gap-2"><span className="size-1.5 rounded-full bg-[#72dece]" /><span className="type-ui font-semibold">Decision trace</span></div>
        <span className="type-meta flex items-center gap-1.5 text-white/50"><CheckCircle2 className="size-3 text-[#72dece]" />Complete</span>
      </div>
        ...
        <div className="p-5 sm:p-6">
          ...
          <div className="mt-5 divide-y divide-white/[0.07] border-y border-white/[0.07]">
            {["Budget available", "Vendor risk cleared", "Finance route selected"].map((item) => (
              <div key={item} className="flex items-center justify-between py-3"><span className="type-meta text-white/55">{item}</span><span className="flex size-5 items-center justify-center rounded-full bg-[#17342e] text-[#72dece]"><Check className="size-3" /></span></div>
            ))}
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 grid grid-cols-2 gap-px overflow-hidden rounded-[7px] border border-white/10 bg-white/10">
              {["Request", "Vendor", "Approval", "Invoice"].map((item) => <div key={item} className="type-meta flex items-center gap-2 bg-[#111714] px-3 py-3 text-white/60"><FileCheck2 className="size-3.5 text-[#72dece]" />{item}</div>)}
            </div>
          </div>

          <div className="type-meta mt-6 flex items-center justify-between border-t border-white/10 pt-4"><div><p className="uppercase text-white/35">Trace ID</p><p className="mt-1 font-medium">VL-2048-AX</p></div><span className="font-semibold text-[#72dece]">Audit-ready</span></div>
        </div>
      </div>
    </div>
  );
