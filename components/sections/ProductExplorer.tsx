    ...
    <section id="workflow" className="bg-[#edf2f0] py-20 text-[#111714] sm:py-28">
      <Container>
        <Reveal className="grid gap-5 md:grid-cols-[0.72fr_1.28fr] md:items-end">
          <p className="type-eyebrow font-semibold uppercase text-[#087f70]">Product architecture</p>
          <div>
            <h2 className="max-w-[700px] text-[length:var(--fs-h2)] font-semibold">{productExplorer.heading}</h2>
            <p className="type-section-copy mt-4 max-w-[640px] text-[#53605a]">{productExplorer.subhead}</p>
          </div>
        </Reveal>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="type-meta font-semibold text-[#7a8580]">0{index + 1}</span>
                  </div>
                  <div className="mt-auto pt-8">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="type-card-title font-semibold">{product.name}</h3>
                      <ArrowRight className="size-3.5 text-[#087f70] transition-transform group-hover:translate-x-0.5" />
                    </div>
                    <p className="type-card-copy mt-2 text-[#66716c]">{product.scope}</p>
                    <div className="mt-4 flex items-start gap-2 border-t border-black/[0.07] pt-3">
                      ...
                      <span className="type-ui text-[#53605a]">{product.policy}</span>
                    </div>
                  </div>
            ...
            <div className="grid gap-9 md:grid-cols-[0.86fr_1.14fr] md:items-center">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="type-eyebrow font-semibold uppercase text-[#72dece]">Policy-aware control layer</p>
                    <p className="mt-1 text-[length:var(--fs-h3)] font-semibold">Context becomes a governed decision.</p>
                  </div>
                </div>
                <p className="type-card-copy mt-4 max-w-[410px] text-white/55">The system understands the request, vendor, budget, role, and transaction before applying the right rule.</p>
                <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2">
                  {productExplorer.foundation.map((item) => (
                    <span key={item} className="type-meta flex items-center gap-1.5 text-white/55"><span className="size-1 rounded-full bg-[#55d7c4]" />{item}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 border-y border-white/10 sm:border-y-0 sm:border-l">
                {productExplorer.policyFlow.map((item, index) => (
                  <div key={item.title} className="relative min-w-0 border-r border-white/10 px-3 py-5 last:border-r-0 sm:px-4">
                    <span className="type-meta font-semibold text-[#72dece]">0{index + 1}</span>
                    <p className="type-ui mt-3 font-semibold">{item.title}</p>
                    <p className="type-meta mt-2 text-white/45">{item.detail}</p>
        ...
        <div className="mt-6 flex justify-end">
          <a href={productExplorer.cta.href} className="type-ui inline-flex items-center gap-2 font-semibold text-[#087f70]">
